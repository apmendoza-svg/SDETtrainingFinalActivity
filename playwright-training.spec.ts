...
test.beforeEach(async ({ page }) => {
  await page.goto("/playwright-training-site.html");
});
...
test.describe("Registration Form", () => {
  test("successful registration with all fields", async ({ page }) => {
    ...
    await page
      .getByTestId("reg-bio")
      .fill("QA engineer with 5 years of experience.");
...
test.describe("Tabs", () => {
  ...
  test("switching tabs shows correct panel", async ({ page }) => {
    ...
    await expect(page.getByTestId("panel-features")).toContainText("Auto-wait");
  });
...
test.describe("Dynamic Content", () => {
  ...
  test("timer increments when started", async ({ page }) => {
    ...
    const value = Number(await page.getByTestId("timer-value").textContent());
  ...
  test("progress bar fills to 100%", async ({ page }) => {
    ...
    await expect(page.getByTestId("progress-fill")).toHaveCSS("width", /.+/);
  });
...
test.describe("Drag and Drop", () => {
  test("initial state has 3 items in To Do", async ({ page }) => {
    const todoItems = page.getByTestId("drag-todo").locator(".drag-item");
    ...
    await expect(
      page.getByTestId("drag-done").locator(".drag-item"),
    ).toHaveCount(0);
  });
  ...
  test("drag item from To Do to In Progress", async ({ page }) => {
    ...
    await expect(
      page.getByTestId("drag-todo").locator(".drag-item"),
    ).toHaveCount(2);
  });
